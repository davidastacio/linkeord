...
export function DashboardShell({ mode, title, eyebrow, children }: DashboardShellProps) {
  ...
  const userRole =
    ...
    userProfile?.role === "emprendedor" ? "Emprendedor" :
    userProfile?.role === "delivery" ? "Delivery" : "Usuario";
        ...
        <div className="relative flex min-h-screen flex-col items-center justify-center bg-[#f7faff] p-4 text-center">
          <div className="relative z-10 w-full max-w-md rounded-2xl border border-border bg-white p-8 shadow-[0_20px_60px_rgba(8,26,58,0.06)] md:p-10">
            ...
            <p className="mt-4 text-sm font-semibold leading-relaxed text-slate-600">
              Lo sentimos, <span className="text-blue-600">{userProfile.full_name}</span>. No tienes permisos de administrador.
            </p>
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
              <Button onClick={() => (window.location.href = "/dashboard")} className="flex h-11 items-center justify-center gap-2 rounded-md bg-blue-600 px-5 text-sm font-black text-white hover:bg-blue-700 transition">
                Ir a mi Dashboard
              </Button>
              <Button onClick={handleLogout} variant="outline" className="flex h-11 items-center justify-center gap-2 rounded-md border-slate-200 bg-white px-5 text-sm font-black text-slate-700 hover:bg-slate-50 transition">
                Cerrar sesión
              </Button>
            </div>
          </div>
    ...
    return (
      <div className="relative flex min-h-screen flex-col items-center justify-center bg-[#f7faff] p-4 text-center">
        ...
        <div className="relative z-10 w-full max-w-lg rounded-2xl border border-white/80 bg-white/70 p-8 shadow-[0_20px_60px_rgba(8,26,58,0.08)] backdrop-blur-md md:p-10">
          ...
          <p className="mt-4 text-sm font-semibold leading-relaxed text-slate-600">
            Hola, <span className="text-blue-600">{userProfile.full_name}</span>. Un administrador debe validar tu perfil antes de que puedas comenzar.
          </p>
          <div className="mt-8 rounded-lg bg-blue-50/50 p-4 text-left text-xs font-bold leading-5 text-slate-600 border border-blue-100/40">
            ...
            <p>1. Nuestro equipo verificará tu registro en un plazo máximo de 24 horas.</p>
            <p>2. Se configurará tu entorno según el rol seleccionado (<span className="capitalize">{userProfile.role}</span>).</p>
            <p>3. Una vez aprobada, al recargar podrás acceder de inmediato.</p>
          </div>
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Button onClick={() => window.location.reload()} className="flex h-11 items-center justify-center gap-2 rounded-md bg-blue-600 px-5 text-sm font-black text-white hover:bg-blue-700 transition">
              Recargar Estado
            </Button>
            <Button onClick={handleLogout} disabled={loggingOut} variant="outline" className="flex h-11 items-center justify-center gap-2 rounded-md border-slate-200 bg-white px-5 text-sm font-black text-slate-700 hover:bg-slate-50 transition">
  ...
  return (
    <div className={cn("min-h-screen", isAdmin ? "bg-[#f6f9ff]" : "bg-[#f7faff]")}>
      <aside className={cn("fixed inset-y-0 left-0 z-30 hidden w-72 overflow-y-auto border-r px-5 py-6 lg:block", isAdmin ? "border-white/10 bg-[#071a36] text-white" : "border-[#e7eefb] bg-white text-navy shadow-[18px_0_55px_rgba(8,26,58,0.06)]")}>
        ...
        <div className={cn("mt-8 rounded-lg p-5 text-white", isAdmin ? "navy-panel" : "bg-gradient-to-br from-[#075bff] via-[#123fe8] to-[#071a36] shadow-soft")}>
          ...
          <p className="mt-2 text-sm text-white/78">Nuestro equipo de soporte esta disponible 24/7 para ayudarte.</p>
          <Button variant="secondary" size="sm" className="mt-4 w-full bg-white/14 text-white hover:bg-white/20">Contactar soporte</Button>
        </div>
        <div className="mt-6 border-t border-border/40 pt-4">
          <Button onClick={handleLogout} disabled={loggingOut} variant="ghost" className={cn("w-full justify-start gap-3 px-3 py-2.5 text-sm font-black transition-all rounded-md", isAdmin ? "text-white/60 hover:bg-white/5 hover:text-white" : "text-slate-500 hover:bg-red-50 hover:text-red-600")}>
        ...
        <header className={cn("sticky top-0 z-20 border-b bg-white/90 backdrop-blur-xl", isAdmin ? "border-border" : "border-[#e8effa]")}>
          <div className="flex min-h-20 items-center justify-between gap-4 px-4 sm:px-6 lg:px-8">
            ...
            <div className="flex items-center gap-2">
              {mode === "dashboard" && (
                <Link href="/dashboard/mis-productos" className="hidden md:flex h-9 items-center gap-2 rounded-lg bg-primary px-4 text-sm font-black text-white shadow-sm hover:bg-primary/90 transition-colors">
                  <PackagePlus className="h-4 w-4" />
                  Nuevo pedido
                </Link>
              )}
              <Button variant="outline" size="icon" aria-label="Notificaciones"><Bell className="h-5 w-5" /></Button>
              <Button variant="outline" size="icon" className="hidden sm:inline-flex" aria-label="Mensajes"><MessageSquareText className="h-5 w-5" /></Button>
              <Button variant="outline" size="icon" onClick={handleLogout} disabled={loggingOut} title="Cerrar sesión" className="text-slate-500 hover:bg-red-50 hover:text-red-600 hover:border-red-200">
                <LogOut className="h-5 w-5" />
              </Button>
              <Link href={mode === "admin" ? "/admin/usuarios" : mode === "provider" ? "/provider/perfil" : "/dashboard/perfil"} className="hidden items-center gap-3 pl-2 sm:flex">
